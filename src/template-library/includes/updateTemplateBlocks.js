		...
		if ( block?.attributes ) {
			...
			if ( imageAttributes ) {
				for ( const key of Object.keys( imageAttributes ) ) {
					...
					if (
						key !== 'blockID' &&
						value.hasOwnProperty( 'backgroundImage' ) &&
						value?.backgroundImage
					) {
						const uploadNewImage = await uploadImages(
							value?.backgroundImage
						);
						if ( uploadNewImage ) {
					...
					if (
						...
						const repeaterImages =
							findRepeaterImage( _repeaterValue );
						...
						if ( repeaterImages ) {
							for ( const key in _repeaterValue ) {
								...
								if (
									image &&
									image.hasOwnProperty( 'backgroundImage' ) &&
