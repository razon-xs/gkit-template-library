...
const SinglePage = () => {
	const { singleTemplate, syncLibrary, dispatch, imageImportType, payload } =
		useContextLibrary();
	...
	useEffect( () => {
		...
		fetch( url, {
			...
			.then( ( data ) => {
				if ( data?.posts ) {
					data.posts.sort( ( a, b ) => {
						if ( a.title.toLowerCase().includes( 'home' ) )
							return -1;
						if ( b.title.toLowerCase().includes( 'home' ) )
							return 1;
						return 0;
					} );
	...
	return (
		<div key={ 0 } className="gutenkit-library-single-page">
			{ loading && (
				<div className="gutenkit-library-single-page__spinner">
			...
			<div className="gutenkit-library-single-page__inner">
				{ heroPage && (
					<div
						className="gutenkit-library-single-page__current"
						key={ 0 }
					>
						<div className="gutenkit-library-single-page__current-header">
							<button
								className="gutenkit-library-single-page__current-back"
								onClick={ () => handleBack() }
							>
								<Back /> Back
							</button>
							<h3 className="gutenkit-library-single-page__current-title">
								{ heroPage.title }
							</h3>
						</div>
				...
				<div className="gutenkit-library-single-page__group">
					<div className="gutenkit-library-single-page__group-header">
						<h3 className="gutenkit-library-single-page__group-title">
							{ __(
								`Pages (${ singlePages.length })`,
								...
								singlePages.map( ( page, index ) => {
									return (
										<div
											...
											className={ `gutenkit-library-single-page__group-item ${
												heroPage?.ID === page.ID
													? 'is-active'
													: ''
											}` }
										>
											<div className="gutenkit-library-single-page__group-item-inner">
												...
												<div className="gutenkit-library-single-page__group-item-content">
													<h3 className="gutenkit-library-single-page__group-item-title">
														{ page.title }
													</h3>
												</div>
											</div>
					...
					<div className="gutenkit-library-single-page__pro-notice">
						{ isPremium && ! isProActive && (
							<div className="gutenkit-library-single-page__pro-notice-inner">
								...
								<p>
									{ __(
										'Unlock access to this template, along with a vast array of others, for as little as $49. Start creating effortlessly today!',
										'gutenkit-blocks-addon'
									) }
								</p>
							</div>
