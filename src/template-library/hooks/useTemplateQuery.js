...
const useTemplateQuery = () => {
	const { templateType, dispatch, templates, syncLibrary, searchInput, templatesPage, templatesHasMore, payload } = useContextLibrary();
	...
	useEffect(() => {
		...
		const templatesFetch = async () => {
			try {
				...
				if (templateType === 'templates') {
					if (searchInput === '') {
						const path = addQueryArgs('https://wpmet.com/plugin/gutenkit/wp-json/gkit/v1/layout-manager-api/templates', queryParams);
		...
		const onIntersection = (items) => {
			...
			if ((loaderItem.isIntersecting && templatesHasMore && searchInput === '')) {
				templatesFetch();
			}
		};
