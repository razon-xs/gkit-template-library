...
const LibraryHeader = () => {
	...
	useEffect( () => {
		if ( activeRef.current ) {
			let button = Array.from(
				activeRef.current.querySelectorAll(
					'.gutenkit-library-menu-item'
				)
			);
	...
	const displayNone = showSinglePage
		...
		<div className="interface-interface-skeleton__header gutenkit-library-header">
			<div className="edit-post-header edit-site-header-edit-mode gutenkit-library-header-content">
					...
					{ templateTypeList.map( ( template, index ) => {
						return (
							<Button
								key={ index }
								variant="link"
								className={ `gutenkit-library-menu-item ${
									templateType === template.slug
										? 'is-active'
										: ''
								}` }
				...
				<div className="gutenkit-library-search">
					...
					<div
						className="gutenkit-library-icon"
						style={ displayNone }
					>
						<Button
							variant="tertiary"
							icon={ <Reload /> }
							className={ `gutenkit-library-synchronize ${
								syncLibrary ? 'is-active' : ''
							}` }
							onClick={ () =>
								dispatch( {
					...
					<span
						className="gutenkit-library-separate"
						style={ displayNone }
					></span>
