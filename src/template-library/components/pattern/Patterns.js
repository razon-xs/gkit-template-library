 ...
 */
const Patterns = () => {
	const { dispatch, searchInput, imageImportType, filter } =
		useContextLibrary();
	const { patterns, loading, loadMoreRef, hasMore } = usePatternQuery();
	const { insertBlocks, insertAfterBlock, replaceBlocks } =
	...
	return (
		<>
			...
			<div className="gutenkit-library-list gutenkit-pattern-part">
				...
				{ hasMore && filter.category === 'all' && (
					<button
						className="has-more-data"
						ref={ loadMoreRef }
					></button>
				) }
				{ patterns && patterns.length === 0 && searchInput !== '' && (
					<Empty />
				) }
			</div>
		</>
