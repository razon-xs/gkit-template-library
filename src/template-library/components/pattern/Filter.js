		...
		<div className="gutenkit-library-filter">
			{
				categories && categories.length === 0 && loading ? (
					...
					<div className="gutenkit-library-filter__inner">
						<h3 className="gutenkit-library-filter-title">{__('Category', 'gutenkit-blocks-addon')}</h3>
						<ul className="gutenkit-library-filter-category-list">
							{
								categories.map((category, index) => {
									return (
										<li key={index} className="gutenkit-library-filter-category-list-item">
											<button
												className={`gutenkit-library-filter-category-list-title ${category.slug === filter?.category ? 'is-active' : ''}`}
