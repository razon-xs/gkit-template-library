...
const usePageQuery = () => {
	const { templateType, dispatch, pages, syncLibrary, searchInput, pageTemplatesPage, pageHasMore, payload } = useContextLibrary();
	...
	useEffect(() => {
		...
		const pageFetch = async () => {
			try {
				...
				if (templateType === 'pages') {
					if (searchInput === '') {
						const path = addQueryArgs('https://wpmet.com/plugin/gutenkit/wp-json/gkit/v1/layout-manager-api/pages', queryParams)
		...
		const onIntersection = (items) => {
			...
			if ((loaderItem.isIntersecting && pageHasMore && searchInput === '')) {
				pageFetch();
			}
		};
